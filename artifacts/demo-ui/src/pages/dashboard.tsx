...
import { Loader2, Flame, Brain, Activity as ActivityIcon, CheckCircle, XCircle } from "lucide-react";
...
export function Dashboard() {
  ...
  const [queryId, setQueryId] = useState("student_1");
                ...
                <CardContent className="p-6 flex flex-col items-center justify-center text-center space-y-4">
                  <div className="relative w-24 h-24 flex items-center justify-center">
                    <svg className="absolute inset-0 w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                      ...
                      <circle cx="50" cy="50" r="45" fill="none" stroke="#7c3aed" strokeWidth="8" strokeDasharray="283" strokeDashoffset={283 - (283 * ((dashboardData.xp || 0) % 500)) / 500} className="transition-all duration-1000" strokeLinecap="round" />
                    </svg>
                    <span className="text-2xl font-bold font-mono text-white">{dashboardData.xp || 0}</span>
                  </div>
                  <div className="font-mono text-xs text-purple-400 tracking-widest uppercase font-bold">Experience Points</div>
                </CardContent>
              </Card>
            </motion.div>
              ...
              <Card className="h-full galaxy-card border-t-[3px] border-t-amber-500 shadow-[0_0_30px_rgba(245,158,11,0.15)] bg-white/5 hover:shadow-[0_0_30px_rgba(245,158,11,0.3)]">
                <CardContent className="p-6 flex flex-col items-center justify-center text-center space-y-4">
                  ...
                  <div className="space-y-1">
                    <div className="text-3xl font-bold font-mono text-white">{dashboardData.streak || 0}</div>
                    <div className="font-mono text-xs text-amber-500 tracking-widest uppercase font-bold">Day Streak</div>
                  </div>
                </CardContent>
              </Card>
            </motion.div>
            ...
            <motion.div whileHover={{ scale: 1.02 }} className="h-full">
              <Card className="h-full galaxy-card border-t-[3px] border-t-blue-500 shadow-[0_0_30px_rgba(59,130,246,0.15)] bg-white/5 hover:shadow-[0_0_30px_rgba(59,130,246,0.3)]">
                <CardContent className="p-6 flex flex-col items-center justify-center text-center space-y-4">
                  ...
                  <div className="space-y-1">
                    <div className="text-3xl font-bold font-mono text-white">{dashboardData.concept_count || 0}</div>
                    <div className="font-mono text-xs text-blue-400 tracking-widest uppercase font-bold">Concepts Encountered</div>
                  </div>
                </CardContent>
              </Card>
            </motion.div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {(dashboardData.mastery || []).map((concept: any, idx: number) => {
                  ...
                  return (
                    <div key={idx} className={`p-4 rounded-lg border ${statusColor} ${bg} flex flex-col gap-2`}>
                      <div className="font-mono text-sm font-bold truncate text-white">{concept.concept}</div>
                      <div className="font-mono text-[10px] uppercase tracking-wider">{concept.status.replace("_", " ")}</div>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-4 relative before:absolute before:inset-0 before:ml-2.5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-white/10 before:to-transparent">
                {(dashboardData.recent_activity || []).slice(0, 5).map((activity: any, idx: number) => (
                ...
                {(!dashboardData.recent_activity || dashboardData.recent_activity.length === 0) && (
                  <div className="text-slate-500 font-mono text-sm italic pl-8">No recent activity detected.</div>
                )}
              </div>
            </div>
          ...
          {badgesData && badgesData.badges && (
            <div className="space-y-4 pt-4 border-t border-white/10">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {badgesData.badges.slice(0, 4).map((badge: any, idx: number) => (
                  <div 
                    key={idx} 
                  ...
                  >
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${badge.unlocked ? 'bg-amber-500/20 text-amber-400' : 'bg-white/10 text-white/40'}`}>
                      {badge.unlocked ? <CheckCircle className="w-5 h-5" /> : <XCircle className="w-5 h-5" />}
                    </div>
                    <div className="min-w-0">
                      <div className="font-mono text-xs font-bold text-white truncate">{badge.name}</div>
                      <div className="font-mono text-[10px] text-slate-400 truncate">{badge.unlocked ? 'UNLOCKED' : 'LOCKED'}</div>
                    </div>
                  </div>
                ))}
              </div>
